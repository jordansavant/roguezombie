    ...
    onActivate = [hud, label] (bit::Element* e) {
        ...
        if(hud->state->isShiftModifierDown)
        {
            ...
            if(label->currentPositionSlot)
            {
                ...
                // If there is no open loot window, Aciton Bar or Equipment
                else
                {
                    // Action Bar
                    ...
                    {
                        InventoryEquipmentSlot* equipmentSlot = hud->inventory->getEquipmentSlotForItemQuickEquip(label->itemSchema);
                        if(equipmentSlot && equipmentSlot->acceptsLabel(label))
                        {
                            bool result = label->dropOntoEquipmentSlot(equipmentSlot);
                            label->dropResult(result);
                        }
                    }
...
void InventoryItemLabel::updateReals(sf::RenderWindow &window, sf::Time &gameTime)
{
    ...
    icon->position((int)left, (int)top, z, 64, 64, elementScale);
}
