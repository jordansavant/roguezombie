...
InventoryPositionSlot::InventoryPositionSlot(Hud* hud, unsigned int position, Inventory* inventory, float relativeX, float relativeY, float width, float height, AnchorType anchorType)
    : Frame(hud, relativeX, relativeY, width, height, anchorType, std::bind(&Hud::typicalContainerControl, inventory->hud, std::placeholders::_1,std::placeholders::_2, std::placeholders::_3)),
      position(position), inventory(inventory), equippedItemLabel(NULL)
{
    managesOpacity = true;
}
